...
To regenerate the publication-ready plots from the saved JSON outputs:

```bash
python dagi_plots.py
```

This produces `mutual_information_curve.png`, `moebius_contributions_stack.png`,
and `higher_order_fraction.png` in the repository root, using only the serialized
`dagi_results.json` data (no re-simulation required).
